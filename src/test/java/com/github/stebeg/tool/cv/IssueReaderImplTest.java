...
public class IssueReaderImplTest {
    ...
    private final UrlBuilder urlBuilderMock;
    ...
    public IssueReaderImplTest() {
        this.urlBuilderMock = mock(UrlBuilder.class);
        this.urlConnectionBuilderMock = mock(UrlConnectionBuilder.class);

        final UrlContentReader urlContentReader = new UrlContentReaderImpl(this.urlConnectionBuilderMock);
    ...
    public void testGetVolumeIssues() throws IOException {
        ...
        expResultIssue2.setName("The Secret of the Golden Thunderbolts!; Riddle of the Frozen Ghost Town!");
        ...
        final URL url = getClass().getResource("/issues-get-example.json");
        ...
        assertEquals(expResultIssue1.getImage().getOriginalUrl(), resultIssue1.getImage().getOriginalUrl());
        ...
        assertEquals(expResultIssue2.getImage().getOriginalUrl(), resultIssue2.getImage().getOriginalUrl());
    ...
    public void testGetIssue() throws IOException {
        ...
        expResultIssue.setDescription("<p><i>The conclusion to Sinestro and Hal Jordan's invasion of the planet Korugar!</i></p>");
        ...
        expResultIssue.getCharacters().add(new Character(18276L, "Carol Ferris"));
        expResultIssue.getCharacters().add(new Character(6951L, "Ganthet"));
        expResultIssue.getCharacters().add(new Character(11202L, "Hal Jordan"));
        expResultIssue.getCharacters().add(new Character(10448L, "Sinestro"));

        expResultIssue.getTeams().add(new Team(6992L, "Green Lantern Corps"));
        expResultIssue.getTeams().add(new Team(42352L, "Sinestro Corps"));

        expResultIssue.getPersonList().add(new Person(1533, "Doug Mahnke", "penciler, cover"));
        expResultIssue.getPersonList().add(new Person(40439, "Geoff Johns", "writer"));

        final URL url = getClass().getResource("/issue-get-example.json");
        ...
        assertEquals(expResultIssue.getImage().getOriginalUrl(), resultIssue.getImage().getOriginalUrl());
        ...
        for (int index = 0; index < resultIssue.getCharacters().size(); index++) {
            assertEquals(expResultIssue.getCharacters().get(index).getId(), resultIssue.getCharacters().get(index).getId());
            assertEquals(expResultIssue.getCharacters().get(index).getName(), resultIssue.getCharacters().get(index).getName());
        }

        assertEquals(expResultIssue.getTeams().size(), resultIssue.getTeams().size());
        for (int index = 0; index < resultIssue.getTeams().size(); index++) {
            assertEquals(expResultIssue.getTeams().get(index).getId(), resultIssue.getTeams().get(index).getId());
            assertEquals(expResultIssue.getTeams().get(index).getName(), resultIssue.getTeams().get(index).getName());
        }
        ...
        for (int index = 0; index < resultIssue.getPersonList().size(); index++) {
            assertEquals(expResultIssue.getPersonList().get(index).getId(), resultIssue.getPersonList().get(index).getId());
            assertEquals(expResultIssue.getPersonList().get(index).getName(), resultIssue.getPersonList().get(index).getName());
            assertEquals(expResultIssue.getPersonList().get(index).getRole(), resultIssue.getPersonList().get(index).getRole());
        }
    }
}

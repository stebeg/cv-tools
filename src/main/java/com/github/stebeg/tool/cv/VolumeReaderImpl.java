...
class VolumeReaderImpl implements VolumeReader {
    ...
    /**
     * Creates a new instance of the implementation for retrieving volumes from
     * the Comicvine API.
     *
     * @param urlBuilder Needed for building the Comicvine API URL for reading
     * volumes.
     * @param urlContentReader Reads the response from the Comicvine API.
     * @param gson Builds objects from the JSON response retrieved from the
     * Comicvine API.
     */
    VolumeReaderImpl(
    ...
    public VolumeSearchResult searchVolumes(
        ...
        final String urlEncodedSearchText = URLEncoder.encode(searchText, Charsets.UTF_8.name());
        final Map<String, String> parameter = ImmutableMap.<String, String>builder()
                ...
                .put(QUERY_PARAMETER_NAME, urlEncodedSearchText)
                .build();
        ...
        final JsonElement jsonElement = jsonParser.parse(jsonContent);
